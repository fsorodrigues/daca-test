        ...
        .append("rect")
          ...
          .on("click", function(d) {
              ...
              if (selection == "box active") {
                  ...
                  var selectionId = d3.select(this).attr("id")

                  var rect = document.getElementById(selectionId).getBoundingClientRect();
...
function showTooltip() {
    var rect = document.getElementById("entry-1").getBoundingClientRect();

    var pos = {top: (rect.top + window.scrollY) , left: (rect.left + window.scrollX) };

    d3.selectAll(".box").attr("class", "box inactive");

    var selection = d3.select("#entry-1")
